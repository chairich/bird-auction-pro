...
export default function AuctionDetail() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const docRef = doc(db, "auctions", String(id));
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      <img src={auction.image} alt={auction.name} className="w-full h-64 object-cover rounded mb-4" />
      <h1 className="text-2xl font-bold">{auction.name}</h1>
      <Countdown endsAt={auction.endsAt} />
      <p>เริ่มต้น: ฿{auction.startPrice} | ปัจจุบัน: ฿{auction.currentBid}</p>
      ...
      <button onClick={handleBid} disabled={submitting} className="bg-green-600 text-white py-2 px-4 rounded">
        {submitting ? "กำลังประมูล..." : "ยืนยันการประมูล"}
      </button>
      <div className="mt-4">
        <h3 className="text-lg font-semibold">📜 ประวัติการประมูล</h3>
        <ul className="text-sm mt-2 space-y-1">
          {auction.bids?.slice().reverse().map((b: any, i: number) => (
            <li key={i}>💰 {b.amount} บาท - 🕒 {new Date(b.time.seconds * 1000).toLocaleString()}</li>
          )) || <li>ไม่มี</li>}
        </ul>
      </div>
    </div>
